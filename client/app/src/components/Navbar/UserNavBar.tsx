...
export default function NavBar(){
    ...
    <div className="sticky top-0 z-10 px-4 py-4 bg-inherit flex justify-between items-center">
        ...
        <button className="relative w-9" onClick={() => setIsProfileOpen(!isProfileOpen)}>
            <img className={ isProfileOpen ? 'w-9 h-9 rounded-full border-4 border-gray-500' : 'w-9 h-9 rounded-full'} src={user.isManager ? user.picture : profile}/>
            {isProfileOpen && 
            <div className='font-manrope text-sm gap-1 text-white font-semibold flex flex-col items-start absolute top-12 right-0 bg-zinc-700 py-2 rounded-xl'>
                <span className='px-6 hover:bg-zinc-500 w-full py-1 text-left'>{user.username}</span>
                <span className='px-6 hover:bg-zinc-500 w-full py-1 text-left'>Settings</span>
                <div className="w-full pt-1 border-t border-gray-500">
                    <span className='flex hover:bg-zinc-500 px-6 py-1 text-left w-full'>Logout</span>
                </div>
            </div>}
        </button>
