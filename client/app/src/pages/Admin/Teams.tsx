...
const Teams = () => {
  ...
  const currentPost = filteredTeams.slice(firstPostindex, lastPostIndex);
    ...
    const scrape_teams = async () => {
  
      setLoading(true)
      try {
        const res = await axios.get(API_URL + "scrapTeams/")
        const request = await axios.get(API_URL + "scrapPlayersTeams/")
        ...
        setScrapingAlertMsg(`an error happened ${err}`)
      } finally {
        setLoading(false)
      }
    ...
    <div className="h-full w-full flex flex-col bg-gray-900">
      ...
      <div className="bg-black px-7 py-11 my-4 rounded-3xl border-2 border-gray-700 w-[45%] mx-auto flex flex-col items-center gap-11">
        ...
        <div className="relative mx-auto w-[80%]">
          <button onClick={scrapingInsurance ? () => { scrape_teams() } : undefined} className={`w-full ${scrapingInsurance ? 'hover:scale-105' : 'cursor-not-allowed'} transition-all duration-300 font-manrope text-white font-bold bg-red-950 px-4 py-3 rounded-lg border-2 border-gray-600`}>{loading ? <div className="mx-auto w-5 h-5 border-4 border-t-red-600 border-gray-300 rounded-full animate-spin"></div> : 'Scrape the teams'}</button>
        </div>
      </div>
